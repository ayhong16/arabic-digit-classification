...
def _check_precisions(precisions, covariance_type, n_components, n_features):
    ...
    precisions_shape = {
        ...
        "tied_spherical": (n_components,),
        "tied_diag": (n_components, n_features)
    }
    _check_shape(
    ...
    _check_precisions = {
        ...
        "tied_spherical": _check_precision_positivity,
        "tied_diag": _check_precision_positivity,
    }
...
def _estimate_gaussian_covariances_tied_spherical(resp, X, nk, means, reg_covar):
    covar = _estimate_gaussian_covariances_tied_diag(resp, X, nk, means, reg_covar).mean(1)
    num = len(_estimate_gaussian_covariances_diag(resp, X, nk, means, reg_covar))
    stack = np.vstack([covar] * num)
    return stack.mean(1)


def _estimate_gaussian_covariances_tied_diag(resp, X, nk, means, reg_covar):
    tied_covar = _estimate_gaussian_covariances_tied(resp, X, nk, means, reg_covar)
    num = len(_estimate_gaussian_covariances_diag(resp, X, nk, means, reg_covar))
    stack = np.vstack([np.diag(tied_covar)] * num)
    return stack
...
def _estimate_gaussian_parameters(X, resp, reg_covar, covariance_type):
    ...
    covariances = {
        ...
        "tied_diag": _estimate_gaussian_covariances_tied_diag,
        "tied_spherical": _estimate_gaussian_covariances_tied_spherical,
...
def _compute_log_det_cholesky(matrix_chol, covariance_type, n_features):
    ...
    elif covariance_type == "diag" or covariance_type == "tied_diag":
...
def _estimate_log_gaussian_prob(X, means, precisions_chol, covariance_type):
    ...
    elif covariance_type == "spherical" or covariance_type == "tied_spherical":
        precisions = precisions_chol**2
        log_prob = (
...
class GaussianMixture(BaseMixture):
    ...
    _parameter_constraints: dict = {
        **BaseMixture._parameter_constraints,
        "covariance_type": [StrOptions({"full", "tied", "diag", "spherical", "tied_diag", "tied_spherical"})],
    ...
    def _n_parameters(self):
        ...
        elif self.covariance_type == "diag" or self.covariance_type == "tied_diag":
            cov_params = self.n_components * n_features
        elif self.covariance_type == "tied":
            cov_params = n_features * (n_features + 1) / 2.0
        elif self.covariance_type == "spherical" or self.covariance_type == "tied_spherical":
